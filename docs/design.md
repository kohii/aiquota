...
| Crit | 着地予測 `>=140%` または 枯渇間近（下記 runway `<0.8`） | 赤(31) / 🔴 | 大きく早く枯渇 or 残り窓を持たない |

- **枯渇間近の網（runway）**: `pct>=85%` かつ **runway `< 0.8`** のとき Crit。runway = 「残枠が、残っている窓のどれだけを現在のペースで持つか」＝ `(100-pct)/rate/(100-pace)`（`rate = pct/pace`）。1.0 ならちょうどリセット時に枯渇、0.5 なら残り窓の半分でロックアウト。
  - **旧実装の欠陥**: `pct>=85%` を pace 不問で Crit にしていたため、「97% 経過で 85% 使用（着地予測 88%＝理想的）」まで 🔴 になっていた。着地予測は序盤に敏感（`50%/pace 24%` → 208%）だが終盤は鈍く（`95%/pace 90%` → 106%）、どちらも「上限付近に着地」に潰れて区別できない。runway はこの逆で終盤に敏感（前者 0.32 / 後者 0.47 / 理想例 5.7）なので、予測と組で窓の両端を覆える。
  - 閾値を 1.0 ではなく 0.8 にしているのは「ちょうど使い切って着地」（`99%/pace 99%` → runway 1.0）を緑に保つため。それは use-it-or-lose-it の理想形であって非常事態ではない。
- **時計が無い/浅い窓のフォールバック**: `pace<20%` や窓情報の無い枠は runway も予測も信頼できないので、絶対値 `pct>=60%→Warn / >=85%→Crit` で拾う。一方 Loss(青)は早すぎる序盤ノイズを避けるため `pace>=25%` まで出さない（赤は早め・青は慎重、という非対称）。
